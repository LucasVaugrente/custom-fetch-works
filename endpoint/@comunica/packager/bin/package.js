...
const configPath = args.c ? args.c : '.';
const mainModulePath = args.p ? Path.resolve(process.cwd(), args.p) : Path.join(__dirname, '..');
let exportVariableName;
if (args.e) {
    exportVariableName = args.e;
}
const dependencyRegex = /require\('([^']*)'\)/ug;
(0, componentsjs_1.compileConfig)(mainModulePath, configPath, 'urn:comunica:default:Runner', exportVariableName, false, true)
    .then((document) => {
